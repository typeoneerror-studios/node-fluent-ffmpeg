...
exports = module.exports = function Processor(command) {
  ...
  command.prototype._getProgressFromStdErr = function(stderrString, totalDurationSec) {
    ...
    var progress;
    if (ll) {
      progress = ll.split(/frame=([0-9]+)fps=([0-9\.]+)q=([0-9\.\-]+)size=([0-9]+)kBtime=(([0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{2})|[0-9\.]+)bitrate=([0-9\.]+)kbits(\/s)?/ig);
    }

    if (progress && progress.length > 9) {
      // build progress report object
      var ret = {
        frames: parseInt(progress[1], 10),
        currentFps: parseInt(progress[2], 10),
        currentKbps: parseFloat(progress[7]),
        targetSize: parseInt(progress[4], 10),
        timemark: this.ffmpegTimemarkToSeconds((typeof progress[5] != 'undefined') ? parseFloat(progress[5]) : progress[6])
      };
