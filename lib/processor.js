...
exports = module.exports = function Processor(command) {
  ...
  this.saveToFile = function(targetfile, callback) {
    ...
    this._prepare(function(err, meta) {
      var restrictionCheck = self.checkRestrictions(meta);
      if (!restrictionCheck.passed) {
        self.options.logger.error('restriction check not passed: ' + restrictionCheck.toString());
        callback(null, new Error('restriction check not passed: ' + restrictionCheck.toString()));
      } else {
        if (err) {
