...
function FfmpegCommand(args) {
  ...
  function RestrictionResult(results) {
    RestrictionResult.prototype.toString = function() {
      var ret = '';

      if (!this.value.passed) {
        for (var i = 0; i < this.value.results.length; i++) {
          ret += 'section ' + this.value.results[i].section + ': ' + this.value.results[i].message + ' ';
        }
      }
      return ret;
    };
    ...
    this.passed = this.value.passed;
  }

  FfmpegCommand.prototype.checkRestrictions = function(meta) {
    if (!Registry.instance.get('restrictions')) {
      // load restriction rules to registry
      Registry.instance.set('restrictions', JSON.parse(fs.readFileSync(__dirname + '/restrictions.json', 'utf-8')));
    }

    var checkresult = [];

    // TODO: check streaming rules
    var rules = Registry.instance.get('restrictions');

    if (rules.streaming) {
      // check input streaming
      if (rules.streaming.block_input) {
        for (var i = 0; i < rules.streaming.block_input.length; i++) {
          if (meta.video.container === rules.streaming.block_input[i]) {
            checkresult.push({ 'section': 'block_input', 'message': meta.video.container + ' is listed in block_input restriction' });
          }
        }
      }
      // check output streaming
      if (rules.streaming.block_output) {
        for (var i = 0; i < rules.streaming.block_input.length; i++) {
          if (this.options.format === rules.streaming.block_output[i]) {
            checkresult.push({ 'section': 'block_output', 'message': this.options.format + ' is listed in block_output restriction' });
          }
        }
      }
    }

    return new RestrictionResult(checkresult);
  };
